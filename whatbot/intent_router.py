...
INTENT_PEDIDO = "pedido"
INTENT_FAQ = "faq"
INTENT_UNKNOWN = "unknown"

_FOLLOW_UP_VERBS = frozenset({"quero", "prefiro", "gostaria", "pode ser"})
# "modalidade"/"modalidades" are business-vocabulary trigger words a
# customer types (e.g. a sports academy customer asking "quais modalidades
# vocês têm?") — kept as content even though the internal identifier for a
# registered offering is `Item`, not `Modalidade` (see
# tests/test_no_modalidade_leftovers.py).
...
def route_intent(
    ...
    items = facts.resolve_items(user_message, session.item_interesse)
    ...
    if len(norm.split()) <= 4 and any(token in norm for token in greeting_signals):
        return IntentResult(INTENT_GREETING, items=items)
    ...
    if session.topico_atual == INTENT_EXPERIMENTAL or session.aguardando_dados_experimental:
        if extract_days(user_message) or re.search(r"\b\d{1,2}h", norm):
            return IntentResult(INTENT_EXPERIMENTAL, items=items)

    if extract_days(user_message) and any(token in norm for token in _FOLLOW_UP_VERBS):
        if session.item_interesse or session.topico_atual in {
            INTENT_EXPERIMENTAL,
            INTENT_PRECOS,
            INTENT_HORARIOS,
        }:
    ...
    if any(token in norm for token in _LISTING_SIGNALS):
        return IntentResult(INTENT_HORARIOS, items=items)
    ...
    if chosen and chosen != INTENT_GREETING:
        return IntentResult(chosen, items=items)

    if session.topico_atual == INTENT_PRECOS and len(norm.split()) <= 6:
        if items or session.item_interesse:
            return IntentResult(
                INTENT_PRECOS,
                items=items or session.item_interesse,
            )
    ...
    if items:
        if scores.get(INTENT_HORARIOS, 0) > 0 or any(
            ...
            return IntentResult(INTENT_HORARIOS, items=items)
        return IntentResult(INTENT_FAQ, items=items)

    return IntentResult(INTENT_UNKNOWN, items=items)

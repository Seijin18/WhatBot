...
### Requirement: Degradação graciosa sem conceito de modalidades

Um negócio cujo arquivo de conhecimento não tem seção `## Modalidades`
(catálogo de produtos, não turmas com horário) DEVE receber respostas
coerentes nos mesmos fluxos que uma associação com modalidades usa —
nenhum caminho DEVE responder com uma mensagem de estado vazio
("nenhuma modalidade cadastrada") como se fosse uma resposta válida ao
cliente.

#### Scenario: "O que vocês têm?" sem modalidades cadastradas

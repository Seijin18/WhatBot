...
- [x] 1.1 `Database.pausar_bot(external_id: str, *, canal: str | None =
...
- [x] 2.1 Nova intenção `pause` em `whatbot/admin_nlu.py` — regex cobrindo
...
- [x] 2.2 `_resolve_pause` em `whatbot/admin.py`: por telefone direto
...
- [x] 2.3 Branch `"pause"` em `_execute_action` chamando `db.pausar_bot` e
      confirmando ("🔕 Bot pausado para *{label}*. Envie *reativar
      {label}* para retomar." — não "libera o bot para {label}": essa
      frase não bate contra `_REACTIVATE` + `search_contacts_for_admin`
      quando o alvo foi resolvido por nome, ver ressalva do critic)
- [x] 2.4 Contato já pausado (`ia_ativa=False`) não aparece como opção em
      `_resolve_pause` — resposta ao admin indica que já está pausado, sem
      erro

## 3. Testes

- [x] 3.1 `pausar_bot` desativa `ia_ativa` de um contato existente e
      retorna `True`; retorna `False` para contato inexistente
- [x] 3.2 Comando "pausa o bot para o João" com contato único pausa direto
- [x] 3.3 Nome ambíguo desambigua antes de pausar, mesmo formato de
      `reactivate`
- [x] 3.4 Contato pausado por este comando NÃO é reativado por
      `process_auto_reactivations()` (não tem `bot_resume_at` setado) —
      só volta via "libera o bot" explícito
- [x] 3.5 Comando "libera o bot para o João" (já existente) reativa um
      contato pausado por este novo comando, sem mudança nenhuma no
      comando de reativação
- [x] 3.6 Suíte completa verde (`make test` / `pytest -q`)

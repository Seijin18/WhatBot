...
- [ ] 1.1 Estender `mensagens` em `ensure_schema()` (`whatbot/db.py`):
...
- [ ] 1.2 Nova tabela `media_arquivos` (`id, contact_id, canal, tipo,
...
- [ ] 1.3 `MessageRecord` (dataclass) ganha os novos campos; atualizar
      qualquer lugar que constrói `MessageRecord` manualmente

## 2. Abstração de storage

- [ ] 2.1 `whatbot/storage/base.py` — `StorageBackend` (Protocol):
      `save(key, data, content_type) -> None`, `open(key) -> bytes`,
      `url(key) -> str | None`
- [ ] 2.2 `whatbot/storage/local.py` — `LocalDiskStorage(root_dir)`:
...
- [ ] 2.3 `whatbot/storage/factory.py` — `get_storage_backend()` lê
...
- [ ] 3.1 `whatsapp_cloud_webhook.py`: `classify_whatsapp_cloud_event`
...
- [ ] 3.2 `InboundMessage` (`whatbot/channels/base.py`) ganha campo
      opcional para referência de mídia, sem quebrar canais que não usam
      mídia
- [ ] 3.3 `WhatsAppCloudClient.download_media(media_id) -> tuple[bytes,
      str]` (`whatbot/channels/whatsapp_cloud.py`): `GET /{media-id}`
      (URL temporária) + `GET` autenticado nessa URL
- [ ] 3.4 `whatbot/main.py`: no recebimento de mensagem com mídia — baixa
...
- [ ] 5.1 Autenticação por bearer token (`ADMIN_API_TOKEN`, env var
      nova) nas rotas `/admin/*`
      (→ Requirement "API administrativa exige autenticação")
- [ ] 5.2 `GET /admin/conversas` — lista contatos com última
      mensagem/preview, canal, status bot/humano
- [ ] 5.3 `GET /admin/conversas/{contact_id}/mensagens?before=&limit=` —
...
- [ ] 5.4 `GET /admin/midia/{media_id}` — stream do binário via
      `StorageBackend.open`, `Content-Type` do `mime_type` salvo
- [ ] 5.5 `POST /admin/conversas/{contact_id}/mensagens` — só aceito
